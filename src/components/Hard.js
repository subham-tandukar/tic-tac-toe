...
function Hard() {
  ...
  const checkWinner = (currBoard, isUserMove) => {
    let isDraw = true;
    for (let i = 0; i < winningConditions.length; i++) {
      ...
      if (
        ...
        setIsUserTurn(true);
        setPlayer("X");
        if (isUserMove && currBoard[a] === "X") {
          setWin(true);
          setWinningRow(i);
          setWinner(currBoard[a]);
          setPopup(true);
        } else if (currBoard[a] === "O" && !isUserMove) {
          setWin(true);
          setWinningRow(i);
          setPopup(true);
          setWinner("O");
  ...
  const botMove = () => {
    if (!checkWinner(board)) {
      ...
      // Check if the user can win in the next move and block that move
      for (let i = 0; i < newBoard.length; i++) {
        if (newBoard[i] === null) {
          newBoard[i] = "X";
          if (checkWinner(newBoard, false)) {
            // Reset newBoard[i] to null if it was set to "X"
      ...
      <div className="content-wrapper">
        <div className="content">
          ...
          <div className="game mt-5">
            <h5>
              Single Player <span className="x">(Mode : Hard)</span>{" "}
            </h5>
            {win ? (
              <>
                <p className="player">
            ...
            <div className="game-board">
              <div
                className={`board-row ${winningRow === 0 ? "winning-row" : ""}`}
              >
                {renderSquare(0)}
                {renderSquare(1)}
                {renderSquare(2)}
              ...
              <div
                className={`board-row ${winningRow === 1 ? "winning-row" : ""}`}
              >
                {renderSquare(3)}
                {renderSquare(4)}
                {renderSquare(5)}
              ...
              <div
                className={`board-row ${winningRow === 2 ? "winning-row" : ""}`}
              >
                {renderSquare(6)}
                {renderSquare(7)}
                {renderSquare(8)}
